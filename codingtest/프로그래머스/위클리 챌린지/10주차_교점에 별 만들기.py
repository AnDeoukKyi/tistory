...
def solution(line):
    ...
    answerT = []
    ans = []
    for ls in list(itertools.combinations(range(len(line)), 2)):
        line1 = line[ls[0]]
        line2 = line[ls[1]]
    ...
    minx = min(ans, key=lambda x:x[0])[0]
    miny = min(ans, key=lambda x:x[1])[1]
    maxx = max(ans, key=lambda x:x[0])[0]
    maxy = max(ans, key=lambda x:x[1])[1]
    for Y in range(maxy - miny + 1):
        tmp = []
        for Y in range(maxx - minx + 1):
            tmp.append(".")
        answerT.append(tmp)
    for ls in ans:
        answerT[ls[1] - miny][ls[0] - minx] = "*"
    for ls in answerT:
        answer.append("".join(ls))
    answer.reverse()
    return answer
